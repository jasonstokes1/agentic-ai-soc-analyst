...
def display_threats(threat_list):
    ...
    for threat in threat_list:
        ...
        logs = threat.get('log_lines', [])
        print(f"\nLog Lines ({len(logs)} total, showing up to 3):")
        for log in logs[:3]:
            print(f"  - {log[:160]}")

        iocs = threat.get('indicators_of_compromise', [])
        print(f"\nIndicators of Compromise ({len(iocs)} total, showing up to 8):")
        for ioc in iocs[:8]:
            print(f"  - {ioc}")
        if len(iocs) > 8:
            print(f"  ... and {len(iocs) - 8} more (full list written to _threats.jsonl)")
