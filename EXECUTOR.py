    ...
    for current_model in model_fallback_chain:
        try:
            ...
            if "503" in str(e) or "UNAVAILABLE" in str(e) or "404" in str(e):
                print(f"{Fore.LIGHTYELLOW_EX}⚠️ Model {current_model} unavailable or error encountered. Falling back to next model...{Style.RESET_ALL}")
                continue
            else:
                print(f"{Fore.LIGHTRED_EX}{Style.BRIGHT}🚨ERROR: API Error on model {current_model}: {e}{Style.RESET_ALL}")
                continue
        except Exception as e:
            print(f"{Fore.RED}Unexpected error with {current_model}:\n{e}{Style.RESET_ALL}")
            continue
